...
!		do tt = 1, 1000000
	...
	implicit none
		...
		nx = trim(adjustl(nx))
		ny = trim(adjustl(ny))
		fname1 = 'uv_field_c2_'//trim(nx)//'_'//trim(ny)//'.dat'
		fname2 = 'u_prof_c2_'//trim(nx)//'_'//trim(ny)//'.dat'
		fname3 = 'v_prof_c2_'//trim(nx)//'_'//trim(ny)//'.dat'

		open(11, file = fname1)
		open(12, file = fname2)
		open(13, file = fname3)

		write(11,*) 'VARIABLE = "X" , "Y" , "U" , "V"'
        write(11,*) 'ZONE ' , ', I=' , n , ', J=' , m

		!Writing solutions in the file
		do i = 1, n
			do j = 1, m
				write(11, *) x_c(i), y_c(j), u(i, j), v(i, j)
			end do
		end do
